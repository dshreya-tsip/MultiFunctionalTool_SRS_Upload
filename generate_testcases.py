...
def fill_excel_template(test_cases, template_path: str, output_path: str, component_name: str):
    ...
    # --- helper to update header fields safely (handles merged cells) ---
            ...
            for c in range(1, search_cols + 1):
                ...
                if isinstance(cell.value, str):
                    ...
                    if text.lower().startswith(label_low + ":"):
                        # If in a merged cell, update only the anchor
                        for rng in ws.merged_cells.ranges:
                            if cell.coordinate in rng:
                                anchor = ws.cell(rng.min_row, rng.min_col)
                                prefix = anchor.value.split(":", 1)[0]
                                anchor.value = f"{prefix}: {value}"
                                return True
                        # Non-merged cell
    ...
    def ensure_header(label: str, value: str, fallback_cell: str):
        if not set_header_field(label, value):
            ws[fallback_cell] = f"{label}: {value}"
 
    # Place headers starting at E2 downward
    ensure_header("Component", component_name, "E2")
    ensure_header("MFP", "Any", "E3")
    ensure_header("Build", "", "E4")
    ensure_header("Date", "", "E5")
    ensure_header("Target", "", "E6")
 
    # Write test cases (table)
    start_row = 8  # table now starts below headers
